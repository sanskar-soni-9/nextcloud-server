	...
	if (OC::$CLI) {
		...
		while ($job = $jobList->getNext($onlyTimeSensitive, $jobClasses)) {
			...
			if ($memoryAfter - $memoryBefore > 10_000_000) {
				$logger->debug('Used memory grew by more than 10 MB when executing job ' . $jobDetails . ': ' . Util::humanFileSize($memoryAfter). ' (before: ' . Util::humanFileSize($memoryBefore) . ')', ['app' => 'cron']);
			}
